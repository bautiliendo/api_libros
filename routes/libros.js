...
router.get("/:id" , (req, res, next) => {
    try{
        ...
        if(!libro) { //En caso de que NO exista 
            const error = new Error('Libro no encontradop');
            error.status = 404;
            throw error;
        }
        res.json(libro) //En caso de que exista 
...
router.delete("/:id" , (req , res, next) => {
    try {
        ...
        const libroEliminado = libros.splice(index, 1); //eliminar un elemento desde el indice (index)
